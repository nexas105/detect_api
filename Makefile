...
env: ## Interactive setup — creates .env.local + .env.prod
...
dev-local: ## Run all services locally (uses .env.local)
	@set -a && . .env.local && set +a && \
	mkdir -p db/data && \
...
dev-docker: ## Docker without SSL (uses .env.prod)
	@mkdir -p db/data
	docker compose --env-file .env.prod up --build -d

prod: ## Production: Docker + Traefik + SSL + Redis (set DOMAIN_* in .env.prod)
	@mkdir -p db/data
...
migrate: ## Migrate SQLite data to PostgreSQL (requires running postgres)
